...
{
    public class ProjectStructDraftRepo : GenericRepository<T_PS_PROJECT_STRUCT_DRAFT>, IProjectStructDraftRepo
    {
        public ProjectStructDraftRepo(NHUnitOfWork unitOfWork) : base(unitOfWork.Session)
        {

        }

        public override IList<T_PS_PROJECT_STRUCT_DRAFT> Search(T_PS_PROJECT_STRUCT_DRAFT objFilter, int pageSize, int pageIndex, out int total)
        {
            var query = Queryable();
        ...
        public IEnumerable<T_PS_PROJECT_STRUCT_DRAFT> SearchWithLinkBoq(IEnumerable<Guid> projectIds, string elementName, IEnumerable<Guid> structureIdInContracts, bool hasFilterVendor)
        {
            var query = Queryable();
