...
{
    public class VolumeWorkRepo : GenericRepository<T_PS_VOLUME_WORK>, IVolumeWorkRepo
    {
        public VolumeWorkRepo(NHUnitOfWork unitOfWork) : base(unitOfWork.Session)
        {
        }

        public override IList<T_PS_VOLUME_WORK> Search(T_PS_VOLUME_WORK objFilter, int pageSize, int pageIndex, out int total)
        {
            var query = Queryable();
        ...
        public IList<T_PS_VOLUME_WORK> GetWithDetails(IEnumerable<Guid> projectIds, string status, bool? isCustomer)
        {
            var query = NHibernateSession.QueryOver<T_PS_VOLUME_WORK>();
