...
{
    public class ProjectStructVersionRepo : GenericRepository<T_PS_PROJECT_STRUCT_VERSION>, IProjectStructVersionRepo
    {
        public ProjectStructVersionRepo(NHUnitOfWork unitOfWork) : base(unitOfWork.Session)
        {

        }

        public override IList<T_PS_PROJECT_STRUCT_VERSION> Search(T_PS_PROJECT_STRUCT_VERSION objFilter, int pageSize, int pageIndex, out int total)
        {
            var query = Queryable();
        ...
        public IEnumerable<T_PS_PROJECT_STRUCT_VERSION> SearchWithLinkBoq(IEnumerable<Guid> projectIds, string elementName, IEnumerable<Guid> structureIdInContracts, bool hasFilterVendor)
        {
            var query = Queryable();
