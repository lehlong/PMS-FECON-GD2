...
gantt.config.editor_types.currency_editor = {
    show: function (id, column, config, placeholder) {
        placeholder.innerHTML = $("#tmplCurrency").html();
        $('.selectCurrency').select2({ width: "250px" });
    },
    hide: function () {
    ...
    set_value: function (value, id, column, node) {
        const currencys = gantt.serverList("currency");
        var value = _.chain(currencys)
    ...
    is_valid: function (value, id, column, node) {
        var currentValue = this.get_value(id, column, node);
        const currencys = gantt.serverList("currency")
        return _.find(currencys, ["key", currentValue]);
...
gantt.config.editor_types.exchange_rate_editor = {
    show: function (id, column, config, placeholder) {
        ...
        let html = ''
        if (unitCode === "%") {
            html = "<input class='txtExchangeRate' data-inputmask=\"'alias': 'percentage', 'max': 10000000, 'min': -9999999, 'allowMinus': true, 'groupSeparator': ',','digits': 3, 'autoGroup':true\" type='text' name='" + column.name + "'/>";
        } else {
            html = "<input class='txtExchangeRate' data-inputmask=\"'alias': 'decimal', 'groupSeparator': ',','digits': 3, 'autoGroup':true\" type='text' name='" + column.name + "'/>";
        }
        placeholder.innerHTML = html;
    ...
    get_value: function (id, column, node) {
        let unformattedMask = Inputmask.unmask(getInput(node).value, { alias: 'decimal', groupSeparator: ',', autoGroup: true });     
        return unformattedMask || 0;
    },
    is_changed: function (value, id, column, node) {
        var currentValue = this.get_value(id, column, node);
...
const exchangeRateEditor = { type: "exchange_rate_editor", map_to: "exchangeRate" };
const numberPriceEditor = { type: "price_editor", map_to: "price" };
const currencyEditor = { type: "currency_editor", map_to: "currency", }
...
function formatTotalVnd(obj) {
    const { total, exchangeRate } = obj || {};
    return Number(total * exchangeRate).toLocaleString("en-US", { maximumFractionDigits: 0 })
}
...
function showCurrencyName(obj) {
    const { currency } = obj || {};
    const currencys = gantt.serverList("currency")
    if (currency) {
        return _.find(currencys, ["key", currency])?.label
    }
}
...
function formatExchangeRate(obj) {
    const { exchangeRate, unitCode } = obj || {};
    if (exchangeRate) {
        return numeral(exchangeRate).format('0,0.[000]');
    }
}
...
const getGanttColumns = () => {
    const columns = [
        ...
        { name: "code", label: "Code", width: 100, resize: true },
        {
            name: "text", label: "Hạng mục", tree: true, width: '*', min_width: 300, resize: true,
        ...
        { name: "currency", label: "Loại tiền", resize: true, align: "right", editor: currencyEditor },
        { name: "exchangeRate", label: "Tỷ giá", resize: true, editor: exchangeRateEditor, template: formatExchangeRate, align: "right" },  
        { name: "totalVnd", label: "Thành tiền (VNĐ)", resize: true, template: formatTotalVnd, align: "right" },  
    ]
