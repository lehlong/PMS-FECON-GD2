...
{
    public class DivisionService : GenericService<T_MD_DIVISION, DivisionRepo>
    {
        ...
        public void Synchronize()
        {
            try
            {
                ...
                {
                    var result = conn.ExecuteFunction(new SynMD_Division_Function() { Parameters = new { I_SALEORG = 6610 } }).ToList();

                    SqlConnection objConn = new SqlConnection(ConfigurationManager.ConnectionStrings["SMO_MSSQL_Connection"].ConnectionString);
                    objConn.Open();

                    string tableName = "T_MD_DIVISION";
