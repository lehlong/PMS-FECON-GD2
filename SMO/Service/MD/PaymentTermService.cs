...
{
    public class PaymentTermService : GenericService<T_MD_PAYMENT_TERM, PaymentTermRepo>
    {
        public PaymentTermService() : base()
        {
                    ...
                    systemConfig.ObjDetail.SAP_USER_NAME, systemConfig.ObjDetail.SAP_PASSWORD))
                {
                    ...
                    foreach (var item in result)
                    {
                        ...
                        if (row == null)
                        {
                            ...
                            newRow["PAYMENT_TERM_D"] = item.PAYMENT_TERM_D;
                            newRow["PAYMENT_TERM_M"] = item.PAYMENT_TERM_M;
                            ...
                            row["TEXT"].ToString() != item.TEXT ||
                            row["PAYMENT_TERM_D"].ToString() != item.PAYMENT_TERM_D.ToString() ||
                            row["PAYMENT_TERM_M"].ToString() != item.PAYMENT_TERM_M.ToString()
                            )
                        {
                            row.BeginEdit();
                            row["PAYMENT_TERM_D"] = item.PAYMENT_TERM_D;
                            row["PAYMENT_TERM_M"] = item.PAYMENT_TERM_M;
                            row["TEXT"] = item.TEXT;
