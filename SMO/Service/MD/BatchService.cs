...
{
    public class BatchService : GenericService<T_MD_BATCH, BatchRepo>
    {
                    ...
                    systemConfig.ObjDetail.SAP_USER_NAME, systemConfig.ObjDetail.SAP_PASSWORD))
                {
                    ...
                    foreach (var item in result)
                    {
                        ...
                        if (row == null)
                        {
                            ...
                            newRow["TEXTS"] = item.TEXTS;
                            newRow["ACTIVE"] = "N";
                            newRow["CREATE_BY"] = actionBy;
                            newRow["CREATE_DATE"] = actionDate;
                            dataTable.Rows.Add(newRow);
                        }
                        else if (row["TEXT"].ToString() != item.TEXT || row["TEXTS"].ToString() != item.TEXTS)
                        {
                            row.BeginEdit();
                            row["TEXT"] = item.TEXT;
                            row["TEXTS"] = item.TEXTS;
