    ...
    public class ProjectStructService : GenericService<T_PS_PROJECT_STRUCT, ProjectStructRepo>
    {
            ...
            try
            {
                ...
                foreach (var item in data)
                {
                    CurrentRepository.Queryable().Where(x => x.ID == item.Id)
                     ...
                     UnitOfWork.Repository<ProjectStructDraftRepo>().Queryable().Where(x => x.ID == item.Id)
                        .Update(x => new T_PS_PROJECT_STRUCT
                        {
                            TOTAL = item.Total,
                            UPDATE_BY = currentUser,
        ...
        internal void UpdateTasksOrder(UpdateTasksOrderDto tasksOrderDto)
        {
            try
            {
                ...
                for (int i = 0; i < tasksOrderDto.TaskIds.Count; i++)
                {
                    ...
                    UnitOfWork.Repository<ProjectStructDraftRepo>().Queryable().Where(x => x.ID == tasksOrderDto.TaskIds[i])
        ...
        public void Update(TaskDto taskDto)
        {
            try
            {
                ...
                if (currentObj.STATUS != taskDto.Status)
                {
                    ...
                    var draft = UnitOfWork.Repository<ProjectStructDraftRepo>().Get(ObjDetail.ID);
                    draft.GEN_CODE = currentObj.GEN_CODE;
                    draft.TEXT = currentObj.TEXT;
                    draft.PARENT_ID = currentObj.PARENT_ID;
                    UnitOfWork.Repository<ProjectStructDraftRepo>().Update(draft);
        ...
        public override void Create()
        {
            try
            {
                ...
                UnitOfWork.GetSession().Save(new T_PS_PROJECT_STRUCT_DRAFT
                {
                    ID = ObjDetail.ID,
                    GEN_CODE = ObjDetail.GEN_CODE,
                    PARENT_ID = ObjDetail.PARENT_ID,
                    PROJECT_ID=ObjDetail.PROJECT_ID,
                    TEXT = ObjDetail.TEXT,
                    START_DATE = ObjDetail.START_DATE,
                    FINISH_DATE = ObjDetail.FINISH_DATE,
                    BOQ_ID = ObjDetail.BOQ_ID,
                    WBS_ID = ObjDetail.WBS_ID,
                    ACTIVITY_ID = ObjDetail.ACTIVITY_ID,
                    TASK_ID = ObjDetail.TASK_ID,
                    C_ORDER = ObjDetail.C_ORDER,
                    STATUS  = ObjDetail.STATUS,
                    TYPE = ObjDetail.TYPE,
                    UNIT_CODE = ObjDetail.UNIT_CODE,
                    QUANTITY = ObjDetail.QUANTITY,
                    TOTAL = ObjDetail.TOTAL,
                    PLAN_VOLUME = ObjDetail.PLAN_VOLUME,
                    PRICE = ObjDetail.PRICE,
                    ACTIVE = ObjDetail.ACTIVE,
                });
